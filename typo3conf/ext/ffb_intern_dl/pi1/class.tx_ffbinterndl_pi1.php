...
/**
 * Plugin 'Mitglieder Downloadliste' for the 'ffb_intern_dl' extension.
 *
 * @author	 <>
 * @package	TYPO3
 * @subpackage	tx_ffbinterndl
 */
class tx_ffbinterndl_pi1 extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	var $prefixId      = 'tx_ffbinterndl_pi1';		// Same as class name
	var $scriptRelPath = 'pi1/class.tx_ffbinterndl_pi1.php';	// Path to this script relative to the extension dir.
	...
	function listView($content, $conf) {
		...
		$this->internal['results_at_a_time']=\TYPO3\CMS\Core\Utility\MathUtility::forceIntegerInRange($lConf['results_at_a_time'],0,1000,5);		// Number of results to show in a listing.
...
//			$fullTable.=$this->pi_list_browseresults();
	...
	//<div'.$this->pi_classParam('listrow').'>
		$out = '<br class="clear">
			<h3>FFB-interne Downloads</h3>
			<br class="clear">
			'.implode(chr(10),$items).'
			<div class="news_head">
				'.$this->pi_linkToPage('Downloadarchiv','30').'
			</div>
			<div class="news_content news_abschluss">
				<p>Weitere Downloads finden Sie in unserem > '.$this->pi_linkToPage('Downloadarchiv','30').'</p>
			</div>';
		return $out;
	}
	...
	function makeListItem()	{
		$config = array();
		$config['file'] = 'uploads/tx_ffbinterndl/'.$this->getFieldContent('screenshot');
		$config['file.']['maxW'] = '140';
		$config['file.']['maxH'] = '140';
		$config['altText'] = $this->getFieldContent('dateititle');
		$config['titleText'] = $this->getFieldContent('dateititle');
		
		if(strlen($this->getFieldContent('screenshot') > 0)){
//		  $screenshot = $this->cObj->IMAGE($config);
		}
		//if(strlen($this->getFieldContent('datei')) > 0){
		$out='
			<div class="news_head">
				<h4>'.$this->getFieldContent('dateititle').'</h4>
			</div>
			<div class="news_content">
				'.$this-> pi_RTEcssText($this->getFieldContent('beschreibung')).
				'<p class="nomargin">Dateidownload:</p><a target="_BLANK" href="uploads/tx_ffbinterndl/'.$this->getFieldContent('datei').'">'.$this->getFieldContent('datei').'</a>
			</div>';
		/*}else{
		$out = '';
		}*/
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ffb_intern_dl/pi1/class.tx_ffbinterndl_pi1.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ffb_intern_dl/pi1/class.tx_ffbinterndl_pi1.php']);
}
